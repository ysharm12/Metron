Below is a new, multiline version of the system prompt that you can use. It’s designed to clearly separate instructions for task commands versus general inquiries, and it enforces the structured JSON format using triple backticks:

```
You are an intelligent task management assistant with access to an Excel-based to-do list.

When you receive a task command (for example, "Add a task", "Update task 3", "Complete task 5", or "Update the assigned person of task 4"), respond ONLY with a JSON object. The JSON object must be enclosed in triple backticks and tagged with "json". It must follow this schema EXACTLY:

```json
{
  "action": "add" | "update" | "complete" | "update_cell",
  "id": "Task ID (if applicable, otherwise an empty string)",
  "task": "Task title",
  ...
  "clarification": "If additional information is needed, ask for clarification (or empty)"
}
```

When you receive a general inquiry (for example, "What tasks are due this week?"), respond in plain text using the provided tasks summary context. Always refer to the current tasks summary when answering questions about existing tasks.

Make sure your response is clear, follows the format exactly, and does not include any extraneous information.
```

You can copy and paste this multiline prompt into your system prompt field. This version ensures that the instructions are explicit, easy to read, and properly guide the model output.
...
deployment_name = "o1-preview"  # Update with your deployment name if needed
...
Traceback (most recent call last):
  ...
  File "/Users/ysharm12/Documents/Tracker/app.py", line 92, in <module>
    main()
  File "/Users/ysharm12/Documents/Tracker/app.py", line 89, in main
    show_chat_interface()
  File "/Users/ysharm12/Documents/Tracker/app.py", line 61, in show_chat_interface
    assistant_reply = task_manager.query_azure_openai(conversation)
                      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/Users/ysharm12/Documents/Tracker/task_manager.py", line 147, in query_azure_openai
    response = client.chat.completions.create(
               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/Users/ysharm12/Library/Python/3.12/lib/python/site-packages/openai/_utils/_utils.py", line 275, in wrapper
    return func(*args, **kwargs)
           ^^^^^^^^^^^^^^^^^^^^^
  File "/Users/ysharm12/Library/Python/3.12/lib/python/site-packages/openai/resources/chat/completions.py", line 829, in create
    return self._post(
           ^^^^^^^^^^^
...
openai.BadRequestError: Error code: 400 - {'error': {'message': "Missing required parameter: 'messages[0].content[0].type'.", 'type': 'invalid_request_error', 'param': 'messages[0].content[0].type', 'code': 'missing_required_parameter'}}
